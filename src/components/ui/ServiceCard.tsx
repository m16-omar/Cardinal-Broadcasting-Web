...
export const ServiceCard: React.FC<ServiceCardProps> = ({ service, index }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col justify-between p-7 sm:p-8 rounded-2xl bg-[#0E1017] border border-white/10 hover:border-[#D92039]/40 transition-all duration-300 shadow-lg hover:shadow-2xl hover:shadow-[#9B111E]/10"
    >
      <div>
        {/* Top Number & Icon */}
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-xs font-mono font-bold text-slate-500 group-hover:text-[#D92039] transition-colors">
            {service.number}
          </span>
        </div>

        {/* Title & Short Description */}
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-3 tracking-tight group-hover:text-[#D92039] transition-colors">
          {service.title}
        </h3>
      ...
      <div className="mt-8 pt-4 border-t border-white/10">
        <Link
          to="/what-we-do"
          className="inline-flex items-center gap-2 text-xs font-semibold text-slate-300 hover:text-white group/link transition-colors"
        >
